...
<div class="page-bg">
    <div style="max-width:640px;margin:0 auto;">
        ...
        <div class="form-card">
            <div class="form-header"><h2>➕ Tambah Santri Baru</h2><p>Isi data santri dengan lengkap dan benar</p></div>
            <div class="form-body">
                <form action="{{ route('admin.santri.store') }}" method="POST">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="nis" value="{{ old('nis') }}" required class="form-ctrl" placeholder="Contoh: 250103001" style="font-family:monospace;">
                            @error('nis')<span class="form-error">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group">
                            <label class="form-label">Tanggal Masuk</label>
                            <input type="date" name="tanggal_masuk" value="{{ old('tanggal_masuk') }}" class="form-ctrl">
                        </div>
                        <div class="form-group full">
                            <label class="form-label">Nama Lengkap *</label>
                            <input type="text" name="nama" value="{{ old('nama') }}" required class="form-ctrl" placeholder="Nama lengkap santri">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <select name="gender" id="gender" required class="form-ctrl" onchange="updateKelas()">
                                ...
                                <option value="PA" {{ old('gender')==='PA'?'selected':'' }}>PA (Putra)</option>
                        ...
                        <div class="form-group">
                            ...
                            <select name="jenjang" id="jenjang" required class="form-ctrl" onchange="updateKelas()">
                                <option value="">-- Pilih --</option>
                                <option value="Reguler" {{ old('jenjang')==='Reguler'?'selected':'' }}>Reguler</option>
                                <option value="Tahfidz" {{ old('jenjang')==='Tahfidz'?'selected':'' }}>Tahfidz</option>
                            </select>
                        </div>
                        <div class="form-group full">
                            <label class="form-label">Kelas *</label>
                            <select name="kelas" id="kelas" required class="form-ctrl">
                                <option value="">-- Pilih gender & jenjang dulu --</option>
                            </select>
                    ...
                    <div class="form-grid">
                        <div class="form-group full">
                            ...
                            <select name="status" required class="form-ctrl">
                                ...
                                <option value="wafat" {{ old('status')==='wafat'?'selected':'' }}>Wafat</option>
                            </select>
                        </div>
                        <div class="form-group full">
                            <label class="form-label">Keterangan</label>
                            <textarea name="keterangan" rows="3" class="form-ctrl" placeholder="Catatan tambahan (opsional)...">{{ old('keterangan') }}</textarea>
                        </div>
                    </div>
                    <div style="margin-top:24px;"><button type="submit" class="btn-submit">Simpan Santri</button></div>
                </form>
            </div>
        </div>
    </div>
...
function updateKelas() {
    ...
    if (!gender || !jenjang) return;
    (KELAS[gender]?.[jenjang] ?? []).forEach(k => {
        const opt = document.createElement('option');
        opt.value = k; opt.textContent = k;
        if ('{{ old('kelas') }}' === k) opt.selected = true;
