...
.profile.spp-profile { background:linear-gradient(135deg,#f0fdf4,#dcfce7); border-bottom:1px solid #bbf7d0; }
...
/* SPP info box */
.spp-info { margin:0 20px 16px; padding:14px; border-radius:12px; background:#ecfdf5; border:1px solid #bbf7d0; }
.spp-info-title { font-size:.75rem; font-weight:700; color:#059669; text-transform:uppercase; letter-spacing:.04em; margin-bottom:6px; }

.notice-done { margin:0 20px 16px; padding:12px; border-radius:10px; background:#f0fdf4; color:#166534; border:1px solid #86efac; font-size:.8rem; font-weight:600; display:none; }
...
/* Role guard banner */
.role-guard { background:linear-gradient(135deg,#fef3c7,#fffbeb); border:1px solid #fcd34d; border-radius:14px; padding:16px 20px; margin-bottom:16px; text-align:center; }
</style>
...
<div class="page">
    <div style="max-width:480px;margin:0 auto;">
        ...
        @if(!Auth::user()->isSuperAdmin())
            {{-- Guard: halaman ini hanya untuk superadmin --}}
            <div class="role-guard">
                <div style="font-size:2rem;margin-bottom:8px;">🔒</div>
                <div style="font-weight:700;color:#92400e;font-size:.95rem;">Akses Terbatas</div>
                <div style="color:#a16207;font-size:.82rem;margin-top:4px;">
                    Halaman TTD SPP/Kantor hanya dapat diakses oleh <strong>Superadmin</strong>.
                </div>
                <a href="{{ route('dashboard') }}"
                   style="display:inline-block;margin-top:12px;padding:8px 20px;border-radius:10px;background:#d97706;color:#fff;font-size:.82rem;font-weight:700;text-decoration:none;">
        ...
        <div class="card">
            <div class="card-header">
                <h2>🏢 TTD SPP / Kantor</h2>
                <p>Scan QR atau ketik NIS untuk verifikasi lunas SPP santri</p>
            </div>
            <div class="card-body">
                ...
                <div class="quick-links">
                    <a href="{{ route('perpulangan.scan.musrif') }}" class="quick-link">✏️ Ke Scan Musrif</a>
                    <a href="{{ route('admin.perpulangan.rekap') }}" class="quick-link">📋 Rekap</a>
                    <a href="{{ route('perpulangan.scan.keamanan') }}" class="quick-link">🛡️ Ke Scan Keamanan</a>
                </div>
            </div>
        </div>
        ...
        <div id="santri-result" style="display:none;">
            <div class="santri-card">
                ...
                <div id="already-notice" class="notice-done">
...
async function doApprove() {
    ...
    try {
        const res  = await apiFetch('/approve/spp','POST',{
            nis    : currentData.nis,
            catatan: document.getElementById('catatan').value.trim()||null,
        });
        const data = await res.json();
        if(data.success) { showToast('success','✅ TTD SPP berhasil!'); renderCard(data.data); document.getElementById('catatan').value=''; }
        else showToast('error', data.message||'Gagal');
    } catch(e) { showToast('error', e.message || 'Gagal menghubungi server'); }
    finally { btn.disabled=false; btn.textContent='🏢 Berikan TTD SPP'; }
}
